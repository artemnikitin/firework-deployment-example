...
resource "google_compute_instance" "role" {
  ...
  metadata_startup_script = templatefile("${path.module}/templates/controlplane-startup.sh.tpl", {
    ...
    controlplane_binary_uri   = "gs://${google_storage_bucket.state.name}/${google_storage_bucket_object.controlplane_binary.name}"
  ...
  depends_on = [
    ...
    google_storage_bucket_object.controlplane_binary,
  ]
}
