...
locals {
  ...
  auto_generate_registry_tls_material = var.auto_create_demo_secrets && var.controlplane_service_mode == "split"
  ...
  effective_registry_tls_cert_secret_id  = var.registry_tls_cert_secret_id != "" ? var.registry_tls_cert_secret_id : (local.auto_generate_registry_tls_material ? google_secret_manager_secret.auto_registry_tls_cert[0].secret_id : "")
  effective_registry_tls_key_secret_id   = var.registry_tls_key_secret_id != "" ? var.registry_tls_key_secret_id : (local.auto_generate_registry_tls_material ? google_secret_manager_secret.auto_registry_tls_key[0].secret_id : "")
...
resource "tls_cert_request" "auto_events_tls" {
  ...
  ip_addresses    = var.controlplane_service_mode == "all" && var.base_domain == "" ? [google_compute_address.registry.address] : []
  dns_names = distinct(compact(concat(
    [trimsuffix(var.events_domain, ".")],
    var.controlplane_service_mode == "all" && var.base_domain != "" ? ["registry.${trimsuffix(var.base_domain, ".")}"] : [],
  )))
...
resource "tls_private_key" "auto_registry_tls" {
  count       = local.auto_generate_registry_tls_material ? 1 : 0
  algorithm   = "ECDSA"
  ecdsa_curve = "P256"
}
